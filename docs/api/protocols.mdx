...
import { RiskScorer } from "@attesta/core";
```
</CodeGroup>
...
```typescript TypeScript
import { Renderer } from "@attesta/core";
```
</CodeGroup>
...
| `_DefaultRenderer` | `attesta.core.gate` (internal) | Auto-approves everything. Used in CI/headless environments when `rich` is not installed or stdin is not a TTY. |

<Note>
Attesta auto-detects the best renderer at runtime. If `rich` is installed and stdin is a TTY (interactive session), the `TerminalRenderer` is used. Otherwise, the silent `_DefaultRenderer` auto-approves all actions. You can override this by passing a `renderer=` parameter to `@gate` or the `Attesta` constructor.
</Note>
...
```typescript TypeScript
import { AuditLogger, ActionContext, ApprovalResult } from "@attesta/core";
...
```typescript TypeScript
import { ChallengeProtocol } from "@attesta/core";
```
</CodeGroup>
