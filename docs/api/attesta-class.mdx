...
| `renderer` | `Renderer` | `DefaultRenderer` | Renderer for presenting challenges. |
| `auditLogger` | `AuditLoggerProtocol` | No-op logger | Audit logger for recording decisions. |
...
| `challenges` | `dict[str, str]` | Alias for `challenge_map` (both are supported). |
