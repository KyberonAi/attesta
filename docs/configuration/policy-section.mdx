...
These mappings are exposed as fields on the `Policy` dataclass: `auto_approve`, `confirm`, `challenge`, and `teach_back`.
...
review_time = policy.min_review_time("high")  # Returns 10
...
# Key methods
policy.challenge_for_risk("high")       # Returns the challenge type for a risk level
policy.min_review_time("critical")      # Returns 30
...
|--------|---------|-------------|
| `challenge_for_risk(level)` | `str` | Challenge type for the given risk level |
