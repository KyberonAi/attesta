...
```
effective_risk = base_risk - (trust_score * influence)
```

For example, an agent with a trust score of `0.8` and default `influence: 0.3`:

```
effective_risk = 0.65 - (0.8 * 0.3) = 0.65 - 0.24 = 0.41
```

The base score of `0.65` (HIGH) is reduced to `0.41` (MEDIUM), changing the challenge from a quiz to a simple confirmation.
...
The `influence` parameter sets the **maximum risk reduction** that trust can provide. It acts as a multiplier on the trust score:

| Trust Score | Influence 0.3 | Influence 0.5 | Influence 0.1 |
|-------------|---------------|---------------|---------------|
| 0.0 (new) | -0.00 | -0.00 | -0.00 |
| 0.3 (initial) | -0.09 | -0.15 | -0.03 |
| 0.5 (moderate) | -0.15 | -0.25 | -0.05 |
| 0.9 (ceiling) | -0.27 | -0.45 | -0.09 |
...
The `decay_rate` parameter controls how quickly trust erodes when an agent is inactive. Trust decays linearly per day of inactivity:

```
trust_after_inactivity = trust_score - (days_inactive * decay_rate)
```
