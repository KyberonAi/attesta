...
<AccordionGroup>
  ...
  <Accordion title="Can I write a custom risk scorer?">
    ...
    import { RiskScorer, ActionContext, gate } from "@attesta/core";
  ...
  <Accordion title="What happens if the operator does not respond to a challenge?">
    ...
    |-------------|---------------------|
    | `deny` (default) | The action is blocked. `AttestaDenied` is raised with verdict `TIMED_OUT`. |
    | `escalate` | The action is forwarded to a secondary approver or webhook. |
    | `allow` | The action proceeds without approval. |
...
<AccordionGroup>
  ...
  <Accordion title="Can I use Attesta in a CI/CD pipeline or headless environment?">
    ...
    - **Default renderer** (auto-detected when no TTY): auto-approves all actions. This is intentional for CI environments where a human already reviewed the code via a pull request.
...
<AccordionGroup>
  <Accordion title="Attesta auto-approves everything without prompting">
    ...
    - Otherwise, the silent default renderer auto-approves everything.
