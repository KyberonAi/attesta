...
| Multi-party challenge | Yes | No | Python-only |
| Trust engine | Yes | Yes | |
| Audit logger (JSONL) | Yes | Yes | |
| Hash-chain verification | Yes | No | Use `attesta audit verify` CLI instead |
...
Both SDKs write to the same `.attesta/audit.jsonl` format. Entries produced by the Python SDK and the TypeScript SDK are fully interoperable -- you can mix and match within the same project, and the `attesta audit verify` command validates the entire chain regardless of which SDK produced each entry.
...
```json
{
  ...
  "risk_score": 0.78,
  "agent_id": "deploy-bot",
  "prev_hash": "e3d5a4f8b2c1..."
}
```

<Warning>
Field names in the audit log JSONL file use `snake_case` regardless of which SDK produced the entry. This is a serialization format decision, not an API convention. The TypeScript SDK maps `camelCase` properties to `snake_case` on write and reverses the mapping on read.
</Warning>
