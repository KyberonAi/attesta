...
# Create an Attesta instance with the custom domain
attesta = Attesta(domain="education")
...
# Verify risk levels (using the risk scorer directly)
scorer = attesta.policy.build_risk_scorer()
